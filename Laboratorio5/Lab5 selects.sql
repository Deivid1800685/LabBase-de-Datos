use Videos4
-- 5 selects
select (vNombre_Cliente +' '+ vApellidoP_Cliente +' '+ vApellidoM_Cliente) as  vNombre_Completo  from Cliente
...
select vNombre_Cliente from Cliente join (select dFecha_Nac, count (*) as FechaDeNacimiento from Cliente Group by dFecha_Nac)subq on Cliente.dFecha_Nac= subq.dFecha_Nac
select count(iID_Cliente) as ID, vNombre_Cliente from  Cliente group by vNombre_Cliente having COUNT (iID_Cliente)=1
...
Select* into DemoBack2 from Demo

drop table Demo

select*into Demo from DemoBack2
select*from Demo

Select *into DemoTemp2
From Demo
INSERT INTO Demo SELECT * FROM DemoTemp2